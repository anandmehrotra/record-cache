...
describe RecordCache::Strategy::Base do
  ...
  context "filter" do
    it "should apply filter on :id cache hits" do
      lambda{ @apples = Apple.where(:id => [1,2]).where(:name => "Adams Apple 1").load }.should use_cache(Apple).on(:id)
      @apples.should == [Apple.find_by_name("Adams Apple 1")]
    end
    
    it "should apply filter on index cache hits" do
      lambda{ @apples = Apple.where(:store_id => 1).where(:name => "Adams Apple 1").load }.should use_cache(Apple).on(:store_id)
      @apples.should == [Apple.find_by_name("Adams Apple 1")]
    end

    it "should return empty array when filter does not match any record" do
      lambda{ @apples = Apple.where(:store_id => 1).where(:name => "Adams Apple Pie").load }.should use_cache(Apple).on(:store_id)
      @apples.should == []
    end
    ...
    it "should filter on integers" do
      lambda{ @apples = Apple.where(:id => [1,2,8,9]).where(:store_id => 2).load }.should use_cache(Apple).on(:id)
      @apples.map(&:id).sort.should == [8,9]
    end

    it "should filter on dates" do
      lambda{ @people = Person.where(:id => [1,2,3]).where(:birthday => Date.civil(1953,11,11)).load }.should use_cache(Person).on(:id)
      @people.size.should == 1
      @people.first.name.should == "Blue"
    end

    it "should filter on floats" do
      lambda{ @people = Person.where(:id => [1,2,3]).where(:height => 1.75).load }.should use_cache(Person).on(:id)
      @people.size.should == 2
      @people.map(&:name).sort.should == ["Blue", "Cris"]
    end

    it "should filter on arrays" do
      lambda{ @apples = Apple.where(:id => [1,2,8,9]).where(:store_id => [2, 4]).load }.should use_cache(Apple).on(:id)
      @apples.map(&:id).sort.should == [8,9]
    end
    ...
    it "should filter on multiple fields" do
      ...
      lambda{ @apples = Apple.where(:id => [1,2,3,8,9,10]).where(:store_id => 2).where(:name => @apple.name).load }.should use_cache(Apple).on(:id)
  ...
  context "sort" do
    it "should apply sort on :id cache hits" do
      lambda{ @people = Person.where(:id => [1,2,3]).order("name DESC").load }.should use_cache(Person).on(:id)
      @people.map(&:name).should == ["Cris", "Blue", "Adam"]
    end

    it "should apply sort on index cache hits" do
      lambda{ @apples = Apple.where(:store_id => 1).order("person_id ASC").load }.should use_cache(Apple).on(:store_id)
      @apples.map(&:person_id).should == [nil, nil, 4, 4, 5]
    end
    
    it "should default to ASC" do
      lambda{ @apples = Apple.where(:store_id => 1).order("person_id").load }.should use_cache(Apple).on(:store_id)
      @apples.map(&:person_id).should == [nil, nil, 4, 4, 5]
    end
    ...
    it "should apply sort nil last for DESC" do
      lambda{ @apples = Apple.where(:store_id => 1).order("person_id DESC").load }.should use_cache(Apple).on(:store_id)
      @apples.map(&:person_id).should == [5, 4, 4, nil, nil]
    end

    it "should sort ascending on text" do
      lambda{ @people = Person.where(:id => [1,2,3,4]).order("name ASC").load }.should use_cache(Person).on(:id)
      @people.map(&:name).should == ["Adam", "Blue", "Cris", "Fry"]
    end

    it "should sort descending on text" do
      lambda{ @people = Person.where(:id => [1,2,3,4]).order("name DESC").load }.should use_cache(Person).on(:id)
      @people.map(&:name).should == ["Fry", "Cris", "Blue", "Adam"]
    end

    it "should sort ascending on integers" do
      lambda{ @people = Person.where(:id => [1,2,3,4]).order("id ASC").load }.should use_cache(Person).on(:id)
      @people.map(&:id).should == [1,2,3,4]
    end

    it "should sort descending on integers" do
      lambda{ @people = Person.where(:id => [1,2,3,4]).order("id DESC").load }.should use_cache(Person).on(:id)
      @people.map(&:id).should == [4,3,2,1]
    end

    it "should sort ascending on dates" do
      lambda{ @people = Person.where(:id => [1,2,3,4]).order("birthday ASC").load }.should use_cache(Person).on(:id)
    ...
    it "should sort descending on dates" do
      lambda{ @people = Person.where(:id => [1,2,3,4]).order("birthday DESC").load }.should use_cache(Person).on(:id)
    ...
    it "should sort ascending on float" do
      lambda{ @people = Person.where(:id => [1,2,3,4]).order("height ASC").load }.should use_cache(Person).on(:id)
      @people.map(&:height).should == [1.69, 1.75, 1.75, 1.83]
    end

    it "should sort descending on float" do
      lambda{ @people = Person.where(:id => [1,2,3,4]).order("height DESC").load }.should use_cache(Person).on(:id)
      @people.map(&:height).should == [1.83, 1.75, 1.75, 1.69]
    end

    it "should sort on multiple fields (ASC + ASC)" do
      lambda{ @people = Person.where(:id => [2,3,4,5]).order("height ASC, id ASC").load }.should use_cache(Person).on(:id)
      @people.map(&:height).should == [1.69, 1.75, 1.75, 1.91]
      @people.map(&:id).should == [4, 2, 3, 5]
    end

    it "should sort on multiple fields (ASC + DESC)" do
      lambda{ @people = Person.where(:id => [2,3,4,5]).order("height ASC, id DESC").load }.should use_cache(Person).on(:id)
      @people.map(&:height).should == [1.69, 1.75, 1.75, 1.91]
      @people.map(&:id).should == [4, 3, 2, 5]
    end

    it "should sort on multiple fields (DESC + ASC)" do
      lambda{ @people = Person.where(:id => [2,3,4,5]).order("height DESC, id ASC").load }.should use_cache(Person).on(:id)
      @people.map(&:height).should == [1.91, 1.75, 1.75, 1.69]
      @people.map(&:id).should == [5, 2, 3, 4]
    end

    it "should sort on multiple fields (DESC + DESC)" do
      lambda{ @people = Person.where(:id => [2,3,4,5]).order("height DESC, id DESC").load }.should use_cache(Person).on(:id)
      @people.map(&:height).should == [1.91, 1.75, 1.75, 1.69]
    ...
    it "should use mysql style collation" do
      ...
      ids << Person.create!(:name => "คฉ Reloady last").id # non-latin characters last in ascending order
      ids << Person.create!(:name => "čedriĉ ꜩ Last").id # latin special, with latin non-collateable

      names_asc = ["1 cedric", "a cedric", "cedric 1", "Cedric 2", "ċedriĉ 3", "čedriĉ 4", "ćedriĉ Last", "čedriĉ คฉ Almost last cedric", "čedriĉ ꜩ Last", "sedric 1", "Sedric 2",  "คฉ Reloady last"]
      lambda{ @people = Person.where(:id => ids).order("name ASC").load }.should hit_cache(Person).on(:id).times(ids.size)
      @people.map(&:name).should == names_asc

      lambda{ @people = Person.where(:id => ids).order("name DESC").load }.should hit_cache(Person).on(:id).times(ids.size)
      @people.map(&:name).should == names_asc.reverse
    end
  end

  it "should combine filter and sort" do
    lambda{ @people = Person.where(:id => [1,2,3]).where(:height => 1.75).order("name DESC").load }.should use_cache(Person).on(:id)
    @people.size.should == 2
    @people.map(&:name).should == ["Cris", "Blue"]

    lambda{ @people = Person.where(:id => [1,2,3]).where(:height => 1.75).order("name").load }.should hit_cache(Person).on(:id).times(3)
