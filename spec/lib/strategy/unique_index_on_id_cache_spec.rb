...
describe RecordCache::Strategy::UniqueIndexCache do
  ...
  context "logging" do
    ...
    it "should write partial hits to the debug log" do
      lambda { Apple.where(:id => [1,2]).load }.should log(:debug, %(UniqueIndexCache on 'id' partial hit for ids [1, 2]: missing [2]))
    end
  end
  ...
  context "cacheable?" do
    ...
    it "should not use the cache when a lock is used" do
      lambda{ Apple.lock("any_lock").where(:id => 1).load }.should_not hit_cache(Apple)
    end

    it "should use the cache when a single id is requested" do
      lambda{ Apple.where(:id => 1).load }.should hit_cache(Apple).on(:id).times(1)
    end

    it "should use the cache when a multiple ids are requested" do
      lambda{ Apple.where(:id => [1, 2]).load }.should hit_cache(Apple).on(:id).times(2)
    end

    it "should use the cache when a single id is requested and the limit is 1" do
      lambda{ Apple.where(:id => 1).limit(1).load }.should hit_cache(Apple).on(:id).times(1)
    end

    it "should not use the cache when a single id is requested and the limit is > 1" do
      lambda{ Apple.where(:id => 1).limit(2).load }.should_not use_cache(Apple).on(:id)
    end

    it "should not use the cache when multiple ids are requested and the limit is 1" do
      lambda{ Apple.where(:id => [1, 2]).limit(1).load }.should_not use_cache(Apple).on(:id)
    end

    it "should use the cache when a single id is requested together with other where clauses" do
      lambda{ Apple.where(:id => 1).where(:name => "Adams Apple x").load }.should hit_cache(Apple).on(:id).times(1)
    end

    it "should use the cache when a multiple ids are requested together with other where clauses" do
      lambda{ Apple.where(:id => [1,2]).where(:name => "Adams Apple x").load }.should hit_cache(Apple).on(:id).times(2)
    end

    it "should use the cache when a single id is requested together with (simple) sort clauses" do
      lambda{ Apple.where(:id => 1).order("name ASC").load }.should hit_cache(Apple).on(:id).times(1)
    end

    it "should use the cache when a multiple ids are requested together with (simple) sort clauses" do
      lambda{ Apple.where(:id => [1,2]).order("name ASC").load }.should hit_cache(Apple).on(:id).times(2)
    end
  end
  ...
  context "record_change" do
    ...
    it "should invalidate destroyed records" do
      lambda{ Apple.where(:id => 1).load }.should hit_cache(Apple).on(:id).times(1)
      @apple1.destroy
      lambda{ @apples = Apple.where(:id => 1).load }.should miss_cache(Apple).on(:id).times(1)
      @apples.should == []
      # try again, to make sure the "missing record" is not cached
      lambda{ Apple.where(:id => 1).load }.should miss_cache(Apple).on(:id).times(1)
    end
    ...
    it "should add updated records to the cache, also when multiple ids are queried" do
      @apple1.name = "Applejuice"
      @apple1.save!
      lambda{ @apples = Apple.where(:id => [1, 2]).order('id ASC').load }.should hit_cache(Apple).on(:id).times(2)
  ...
  context "invalidate" do
    ...
    it "should only miss the cache for the invalidated record when multiple ids are queried" do
      # miss on 1
      Apple.record_cache[:id].invalidate(1)
      lambda{ Apple.where(:id => [1, 2]).load }.should miss_cache(Apple).on(:id).times(1)
      # hit on 2
      Apple.record_cache[:id].invalidate(1)
      lambda{ Apple.where(:id => [1, 2]).load }.should hit_cache(Apple).on(:id).times(1)
    ...
    it "should invalidate records when using update_all" do
      ...
      lambda{ @apples = Apple.where(:id => [1, 2, 3, 4, 5]).order('id ASC').load }.should hit_cache(Apple).on(:id).times(5)
      ...
      lambda{ @apples = Apple.where(:id => [1, 2, 3, 4, 5]).order('id ASC').load }.should hit_cache(Apple).on(:id).times(2)
    ...
    it "should invalidate reflection indexes when a has_many relation is updated" do
      ...
      lambda{ Apple.where(:store_id => 1).load }.should hit_cache(Apple).on(:id).times(2)
